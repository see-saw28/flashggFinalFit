...
parser.add_argument('--logo-sub', default='Preliminary', help='sub-logo text')
...
if len(other_scans) > 0:
    ...
    if args.x_min is not None or args.x_max is not None:
        xmin = args.x_min if args.x_min is not None else axishist.GetXaxis().GetXmin()
        xmax = args.x_max if args.x_max is not None else axishist.GetXaxis().GetXmax()
        axishist.GetXaxis().SetLimits(xmin, xmax)
...
# plot.DrawCMSLogo(pads[0], args.logo, args.logo_sub, 11, 0.085, 0.035, 1.2,  cmsTextSize = 1.)
latex = ROOT.TLatex()
latex.SetNDC()
latex.SetTextFont(61)
latex.SetTextSize(0.055)
latex.DrawLatex(0.16, 0.88, args.logo)

latex.SetTextFont(52)
latex.SetTextSize(0.045)
latex.DrawLatex(0.16, 0.82, args.logo_sub)
